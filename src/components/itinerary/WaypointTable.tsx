        ...
        <table className="w-full text-xs border-collapse min-w-[480px]">
          <thead>
            <tr className="text-[var(--ink-3)] border-b border-[var(--paper-3)]">
              ...
              <th className="text-right py-1.5 px-2 font-medium whitespace-nowrap hidden sm:table-cell">
                {isMetric ? "Elev m" : "Elev ft"}
              </th>
              <th className="text-right py-1.5 px-2 font-medium whitespace-nowrap">↑</th>
              <th className="text-right py-1.5 px-2 font-medium whitespace-nowrap">
                {isMetric ? "km" : "mi"}
              </th>
              <th className="text-right py-1.5 px-2 font-medium whitespace-nowrap">
                {isMetric ? "Total km" : "Total mi"}
              </th>
              <th className="text-right py-1.5 px-2 font-medium whitespace-nowrap">Time</th>
              <th className="text-left py-1.5 pl-2 font-medium hidden md:table-cell">Escape</th>
            </tr>
          </thead>
          <tbody>
            {waypoints.map((wp, i) => {
              if (wp.is_break) {
                return (
                  <tr key={i} className="border-b border-[var(--paper-2)]">
                    <td colSpan={7} className="py-1.5 text-[var(--ink-3)] italic pl-3">
                      ☕ {wp.location}
              ...
              return (
                ...
                >
                  <td className={`py-1.5 pr-3 ${isLast ? "font-semibold" : ""}`}>
                    {wp.location}
                    {hasEscape && (
                      <span className="block text-amber-600 text-[10px] md:hidden">
                        ↪ {wp.escape}
                      </span>
                    )}
                  </td>
                  <td className="text-right py-1.5 px-2 text-[var(--ink-3)] tabular-nums hidden sm:table-cell">
                    {fmtElev(wp.elevation_ft)}
                  </td>
                  <td className="text-right py-1.5 px-2 text-[var(--ink-3)] tabular-nums">
                    {fmtGain(wp.gain_ft)}
                  </td>
                  <td className="text-right py-1.5 px-2 tabular-nums">
                    {fmtDist(wp.dist_mi)}
                  </td>
                  <td className="text-right py-1.5 px-2 tabular-nums text-[var(--ink-2)]">
                    {fmtDist(wp.total_dist_mi)}
                  </td>
                  <td className="text-right py-1.5 px-2 tabular-nums font-mono text-[var(--ink-2)]">
                    {wp.time ?? "—"}
                  </td>
                  <td className="py-1.5 pl-2 text-amber-700 text-[10px] hidden md:table-cell">
                    {wp.escape ?? ""}
                  </td>
                </tr>
              );
